...
const AppointmentPage = () => {
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case "datetime":
        const now = moment(); // 11:40 AM +07, 30/07/2025
        const defaultStartDate = startWeekDate || moment().format('YYYY-MM-DD'); // Mặc định hôm nay nếu chưa chọn
        const twoHoursLater = moment().add(2, 'hours'); // 13:40 PM, 30/07/2025

        return (
          <div className="p-4 bg-white rounded shadow-sm">
            <h3 className="text-primary fw-bold mb-4">Chọn Ngày và Giờ</h3>
            <div className="mb-3">
              <label className="form-label">Chọn ngày bắt đầu</label>
              <input
                type="date"
                className="form-control"
                value={defaultStartDate}
                onChange={(e) => setStartWeekDate(e.target.value)}
                min={moment().format('YYYY-MM-DD')} // Không cho chọn ngày trước hôm nay
              />
            </div>
            {weekSlots.length === 0 ? (
                ...
                <thead>
                  <tr>
                    {Array.from({ length: 7 }).map((_, dayIdx) => {
                      ...
                      const dayFormatted = dayDate.isSameOrAfter(now, 'day')
                        ? dayDate.format('dddd (DD/MM/YYYY)') // Ví dụ: "Wednesday (30/07/2025)"
                        : null; // Không hiển thị nếu trước hôm nay
                      return (
                        <th key={dayIdx} className="text-center bg-primary text-white">
                          {dayFormatted || "Không chọn được"}
                        </th>
                      );
                    })}
                  </tr>
