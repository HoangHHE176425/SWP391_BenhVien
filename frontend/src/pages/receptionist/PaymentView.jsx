...
const PaymentView = () => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      const res = await axios.get("http://localhost:9999/api/receptionist/payments", {
  ...
  const fetchSummary = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("http://localhost:9999/api/receptionist/payments/summary", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setSummary(res.data);
    } catch (err) {
